...
def igual(t):
    ...
    global valores
    if len(valores) > 0 and (valores[-1].isnumeric() or valores[-1] == ')'):
        ...
        if not (valores).is_integer():
            casas_virgula = len(str(valores)) - 1 - str(valores).find(
                '.')  # calculo para descobrir o numero de casas apos a virgula
...
nmros_digitados = Label(frame1, textvariable=t, width=14, height=1, padx=4, pady=13, relief=FLAT, anchor='e',
                        bg='gray90'
                        , justify=RIGHT, font='Ivy 18').place(x=0, y=0)

# Botões das operações
soma = Button(frame2, text='+', command=lambda: escrever_simb(t, ' + '), height=3, width=6, bg='#fa954d').place(x=0,
                                                                                                                y=50)
subtracao = Button(frame2, text='-', command=lambda: escrever_simb(t, ' - '), height=3, width=6, bg='#fa954d').place(
    x=54, y=50)
multiplicacao = Button(frame2, text='x', command=lambda: escrever_simb(t, ' x '), height=3, width=6,
                       bg='#fa954d').place(x=108, y=50)
apagar = Button(frame2, text='⌫', command=lambda: apaga(), height=3, width=6, bg='#fa594d').place(x=162, y=50)
divisao = Button(frame2, text='÷', command=lambda: escrever_simb(t, ' ÷ '), height=3, width=6, bg='#fa954d').place(
    x=162, y=108)
...
# botao de sair
sair = Button(frame2, text='Exit', command=lambda: janela.destroy(), height=3, width=6, bg='#fa594d').place(x=0, y=282)

# Parênteses
abre_prts = Button(frame2, text='(', command=lambda: abrir_parenteses(t), height=3, width=6, bg='#fa954d').place(x=162,
                                                                                                                 y=166)
fecha_prts = Button(frame2, text=')', command=lambda: fechar_parenteses(t), height=3, width=6, bg='#fa954d').place(x=162,
                                                                                                             y=224)
